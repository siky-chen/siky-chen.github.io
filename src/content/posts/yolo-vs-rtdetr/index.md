...
最近在做货物托盘检测，模型初选主要落在 `YOLO` 和 `RT-DETR` 两条路线之间。两者都能完成目标检测任务，但它们背后的设计目标并不一样：`YOLO` 更偏向成熟、稳定、易部署的实时检测范式，`RT-DETR` 则代表了把 `DETR` 思路推向实时场景的一类新方案。  

如果只是看公开榜单，很容易把问题简化成“谁的精度更高”。但在真实项目里，模型选型往往还要一起考虑延迟预算、部署平台、训练成本、后处理复杂度。下面这篇对比，重点就放在这几个工程上真正有影响的维度。
...
<video
  ...
  controls
  preload="metadata"
  src="./rtdetr-ultralytics-demo.mp4"
></video>
